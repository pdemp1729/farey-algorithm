...
from farey.algo import ALLOWED_METHODS, find_rational_approximation
...
def test_rational_approximation_of_irrational():
    ...
    for method in ALLOWED_METHODS:
        r = find_rational_approximation(x, method=method, places=7)
        assert r == Rational(2378, 5741)
        assert_almost_equal(x, float(r), places=7)

    for method in ALLOWED_METHODS:
        r = find_rational_approximation(x, method=method, max_denominator=1000)
...
def test_rational_approximation_zero():
    ...
    for method in ALLOWED_METHODS:
        r = find_rational_approximation(x, method=method, max_denominator=10)
        assert r == Rational(0, 1)
