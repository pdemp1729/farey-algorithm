...
def test_input():
    ...
    with raises(ValueError) as excinfo:
        _ = find_rational_approximation(x, method="continued_fraction")
    assert str(excinfo.value) == "must specify max_denominator"
...
def test_rational_approximation_of_irrational():
    ...
    rf = find_rational_approximation(x, method="farey", max_denominator=1000)
    rc = find_rational_approximation(
        x, method="continued_fraction", max_denominator=1000
    )
    assert rf == rc == Rational(408, 985)
...
def test_rational_approximation_zero():
    ...
    r = find_rational_approximation(x, method="continued_fraction", max_denominator=10)
    assert r == Rational(0, 1)
...
def test_rational_approximation_greater_than_one():
    ...
    assert rf == rc == Rational(2177, 528)
...
def test_rational_approximation_extremal():
    ...
    assert rf == rc == Rational(1, 1000)


def test_continued_fraction_method_full_coverage():
    x = math.sqrt(2)
    r1 = find_rational_approximation(
    ...
    r2 = find_rational_approximation(x, method="continued_fraction", max_denominator=10)
    ...
    x = math.log(3)
    r3 = find_rational_approximation(x, method="continued_fraction", max_denominator=50)
